class ChatController < ApplicationController
  ...
  def create
    ...
    query_embeddings = helpers.fetch_embeddings_query(@query)
    products = Product.new(embedding: query_embeddings).nearest_neighbors(:embedding, distance: "inner_product").first(5)
    ...
    @message = helpers.create_chat_completion(prompt, @chat.messages)["message"]
